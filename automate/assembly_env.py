...
class AssemblyEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: AssemblyEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        if assembly_id := os.environ.get("AUTOMATE_ASSEMBLY_ID"):
            self.cfg_task.assembly_id = assembly_id
            self.cfg_task.assembly_dir = f"{ASSET_DIR}/{assembly_id}/"
            self.cfg_task.disassembly_path_json = f"{self.cfg_task.assembly_dir}disassemble_traj.json"
            self.cfg_task.eval_filename = f"evaluation_{assembly_id}.h5"
            self.cfg_task.fixed_asset.spawn.usd_path = (
                f"{self.cfg_task.assembly_dir}{self.cfg_task.fixed_asset_cfg.usd_path}"
            )
            self.cfg_task.held_asset.spawn.usd_path = (
                f"{self.cfg_task.assembly_dir}{self.cfg_task.held_asset_cfg.usd_path}"
            )
            if (v := os.environ.get("AUTOMATE_IF_SBC")) is not None:
                self.cfg_task.if_sbc = v.lower() in {"1", "true", "yes"}
            if (v := os.environ.get("AUTOMATE_IF_LOG_EVAL")) is not None:
                self.cfg_task.if_logging_eval = v.lower() in {"1", "true", "yes"}
